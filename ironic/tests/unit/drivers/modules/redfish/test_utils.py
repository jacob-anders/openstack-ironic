...
class WatchBootProgressChangeTestCase(db_base.DbTestCase):
    ...
    def test_state_changed(self, mock_get_system, mock_monotonic, mock_sleep):
        mock_monotonic.side_effect = [0, 0]
        mock_get_system.return_value = self._system(
            sushy.BootProgressStates.MEMORY)

        self.assertTrue(redfish_utils.watch_boot_progress_change(
            self.node, sushy.BootProgressStates.OS_RUNNING, 60, 15))

        mock_sleep.assert_called_once_with(15)
        mock_get_system.assert_called_once_with(self.node)

    def test_state_unchanged_until_timeout(self, mock_get_system,
    ...
    def test_read_failure_keeps_watching(self, mock_get_system,
                                         mock_monotonic, mock_sleep):
        mock_monotonic.side_effect = [0, 0, 15]
        mock_get_system.side_effect = [
            exception.RedfishConnectionError(node='node',
                                             error='no route to host'),
            self._system(sushy.BootProgressStates.MEMORY)]

        self.assertTrue(redfish_utils.watch_boot_progress_change(
            self.node, sushy.BootProgressStates.OS_RUNNING, 60, 15))

        self.assertEqual(2, mock_get_system.call_count)

    def test_read_failure_throughout_is_not_evidence(self, mock_get_system,
                                                     mock_monotonic,
                                                     mock_sleep):
        mock_monotonic.side_effect = [0, 0, 15, 30, 45, 60]
    ...
    def test_boot_progress_appears(self, mock_get_system, mock_monotonic,
                                   mock_sleep):
        ...
        self.assertTrue(redfish_utils.watch_boot_progress_change(
            self.node, None, 60, 15))

    def test_boot_progress_absent_throughout(self, mock_get_system,
                                             mock_monotonic, mock_sleep):
        mock_monotonic.side_effect = [0, 0, 30, 60]
        mock_get_system.return_value = mock.Mock(boot_progress=None)

        self.assertFalse(redfish_utils.watch_boot_progress_change(
            self.node, None, 60, 30))

    def test_timeout_zero_skips_the_watch(self, mock_get_system,
                                          mock_monotonic, mock_sleep):
        self.assertFalse(redfish_utils.watch_boot_progress_change(
            self.node, sushy.BootProgressStates.OS_RUNNING, 0, 15))

        mock_get_system.assert_not_called()
        mock_sleep.assert_not_called()
        mock_monotonic.assert_not_called()

    def test_short_timeout_never_sleeps_past_it(self, mock_get_system,
                                                mock_monotonic, mock_sleep):
        ...
        self.assertFalse(redfish_utils.watch_boot_progress_change(
            self.node, sushy.BootProgressStates.OS_RUNNING, 5, 15))
